...
async function getLactanteInfo(lactanteId: string) {
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/lactante/${lactanteId}`, {
    cache: 'no-store'
  });
...
export default async function LactantePage({ params }: { params: { lactanteId: string } }) {
